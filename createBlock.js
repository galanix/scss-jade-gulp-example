// https://gist.github.com/nicothin/1ba505f7cdfbe969600afcad6d9f1d33
...
let defaultExtensions = ['desktop.scss', 'tablet.scss', 'mobile.scss']; // расширения по умолчанию
...
if(blockName) {
  ...
  let dirPath = 'src/scss/components/' + blockName + '/'; // полный путь к создаваемой папке блока
  mkdirp(dirPath, function(err){                 // создаем
    ...
    // Нет ошибки, поехали!
      ...
      extensions.forEach(function(extention){
        ...
        // Если это SCSS
        if(extention == 'scss') {
          styleFileImport = '@import \'./src/scss/components/' + blockName + '/' + blockName + '.css\';';
        }
